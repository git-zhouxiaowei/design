...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            // 导出压缩包
            exportZip: function(formId) {
                table.set();
                $.modal.confirm("确定导出该商户所有信息吗？", function() {
                    ...
                    $.post(table.options.exportZipUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            var aEle = document.createElement("a");// 创建a标签
                            // blob = new Blob([content]);
                            // aEle.download = fileName;// 设置下载文件的文件名
                            //aEle.href = URL.createObjectUrl(blob);
                            aEle.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;;// content为后台返回的下载地址
                            aEle.click();// 设置点击事件
        ...
        operate: {
            ...
            /*后台商户查看详情*/
            detail2: function(shopCode, width, height) {
                var _url=ctx+"system/base/detail/"+shopCode
                table.set();
                var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
                //如果是移动端，就使用自适应大小弹窗
